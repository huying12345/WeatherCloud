...
namespace Yamon.Module.UCenter.DAL
{
    ...
    public partial class UserDAL : _UserDAL
    {
        ...
        //下载用户
        public int DownloadUsersToDB(List<GetMemberResult> list)
        {
            List<User> userList = GetEntityList("", new object[] { }, "", 0, "");

            foreach (GetMemberResult user in list)
            {
                List<User> listTemp = userList.Where(o => o.UserName == user.userid).ToList();

                if (listTemp.Count == 0)
                {
                    User userAdd = new User();

                    userAdd.UserID = GetMaxID();
                    userAdd.UserName = user.userid;
                    userAdd.TrueName = user.name;
                    userAdd.PassWord = MD5Encrypt.MD5("123456");
                    userAdd.Email = user.email != null ? user.email : "";
                    userAdd.Status = user.status;
                    userAdd.DepartmentID = user.department[0];
                    userAdd.Tel = user.mobile;
                    userAdd.Photo = user.avatar;
                    userAdd.Gender = DataConverter.ToInt(user.gender);
                    userAdd.weixinID = user.weixinid;

                    InsertByModel(userAdd);
                }
            }
