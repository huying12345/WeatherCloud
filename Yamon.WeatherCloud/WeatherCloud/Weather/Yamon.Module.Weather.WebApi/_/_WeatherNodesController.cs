...
//     如果想修改此文件的方法，请在此文件的上级目录中的WeatherNodesController重写(override)该方法。
...
namespace Yamon.Module.Weather.WebApi
{
    ...
    /// 共享资源分类
    /// </summary>
    public partial class _WeatherNodesController :BaseController
    {

        public WeatherNodesDAL dal = new WeatherNodesDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(WeatherNodes model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        WeatherNodes newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(WeatherNodes model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.NodeID;
                    if(returnData){
                        WeatherNodes newrow = dal.GetModelByID(model.NodeID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<WeatherNodes> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<WeatherNodes>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
        ...
        public  virtual ActionResult TreeJsonByFilterID(string filterId)
        {
            ...
            try
            {
                ...
                string fields="NodeID,NodeName,ParentID,OrderID";
                ...
                TreeHelper treeHelper = new TreeHelper("NodeID", "NodeName", "OrderID", "ParentID");
        ...
        public  virtual ActionResult AsyncTreeJsonByFilterID(string filterId)
        {
            ...
            try
            {
                ...
                switch(filterId)
                {
                     ...
                     default:
                     {
                        table = dal.GetAsyncTreeList(parentId,"NodeID,NodeName");
                        break;
                     }
                }
                TreeHelper treeHelper = new TreeHelper("NodeID", "NodeName", "OrderID", "ParentID");
        ...
        {
            int result = dal.UpdateSingleFieldValue("NodeName", value,id );
            hash["success"] = result > 0;
            return Json(hash);
        }   
    [LogFilter]
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Weather_WeatherNodes_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            WeatherNodes model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
