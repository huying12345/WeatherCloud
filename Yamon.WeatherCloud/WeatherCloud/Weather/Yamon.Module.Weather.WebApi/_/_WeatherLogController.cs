...
//     如果想修改此文件的方法，请在此文件的上级目录中的WeatherLogController重写(override)该方法。
...
namespace Yamon.Module.Weather.WebApi
{
    ...
    /// 日志
    /// </summary>
    public partial class _WeatherLogController :BaseController
    {

        public WeatherLogDAL dal = new WeatherLogDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(WeatherLog model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        WeatherLog newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(WeatherLog model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.LogID;
                    if(returnData){
                        WeatherLog newrow = dal.GetModelByID(model.LogID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<WeatherLog> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<WeatherLog> modelList = new List<WeatherLog>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Weather_WeatherLog_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            WeatherLog model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
