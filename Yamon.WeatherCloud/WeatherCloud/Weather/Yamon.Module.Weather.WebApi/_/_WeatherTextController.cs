...
//     如果想修改此文件的方法，请在此文件的上级目录中的WeatherTextController重写(override)该方法。
...
namespace Yamon.Module.Weather.WebApi
{
    ...
    /// 文本
    /// </summary>
    public partial class _WeatherTextController :BaseController
    {

        public WeatherTextDAL dal = new WeatherTextDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(WeatherText model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        WeatherText newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(WeatherText model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        WeatherText newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<WeatherText> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<WeatherText> modelList = new List<WeatherText>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<WeatherText>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Weather_WeatherText_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            WeatherText model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
