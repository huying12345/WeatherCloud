...
//     如果想修改此文件的方法，请在此文件的上级目录中的WeatherDocController重写(override)该方法。
...
namespace Yamon.Module.Weather.WebApi
{
    ...
    /// 文档
    /// </summary>
    public partial class _WeatherDocController :BaseController
    {

        public WeatherDocDAL dal = new WeatherDocDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(WeatherDoc model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        WeatherDoc newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(WeatherDoc model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        WeatherDoc newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<WeatherDoc> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<WeatherDoc> modelList = new List<WeatherDoc>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Weather_WeatherDoc_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            WeatherDoc model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
