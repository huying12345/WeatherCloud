...
//     如果想修改此文件的方法，请在此文件的上级目录中的WeatherNodesSubscribeController重写(override)该方法。
...
namespace Yamon.Module.Weather.WebApi
{
    ...
    /// 用户资源订阅
    /// </summary>
    public partial class _WeatherNodesSubscribeController :BaseController
    {

        public WeatherNodesSubscribeDAL dal = new WeatherNodesSubscribeDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(WeatherNodesSubscribe model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        WeatherNodesSubscribe newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(WeatherNodesSubscribe model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        WeatherNodesSubscribe newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<WeatherNodesSubscribe> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<WeatherNodesSubscribe> modelList = new List<WeatherNodesSubscribe>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<WeatherNodesSubscribe>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Weather_WeatherNodesSubscribe_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            WeatherNodesSubscribe model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
