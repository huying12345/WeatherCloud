...
//     如果想修改此文件的方法，请在此文件的上级目录中的TyphoonNameController重写(override)该方法。
...
namespace Yamon.Module.Typhoon.WebApi
{
    ...
    /// 台风名称
    /// </summary>
    public partial class _TyphoonNameController :BaseController
    {

        public TyphoonNameDAL dal = new TyphoonNameDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(TyphoonName model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        TyphoonName newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(TyphoonName model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.id;
                    if(returnData){
                        TyphoonName newrow = dal.GetModelByID(model.id);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<TyphoonName> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<TyphoonName> modelList = new List<TyphoonName>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<TyphoonName>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Typhoon_TyphoonName_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            TyphoonName model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
