...
namespace Yamon.Module.Typhoon.WebApi
{
    ...
    public partial class _TyphoonPathController :BaseController
    {
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(TyphoonPath model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        TyphoonPath newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(TyphoonPath model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        TyphoonPath newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<TyphoonPath> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<TyphoonPath>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Typhoon_TyphoonPath_Show")]        
