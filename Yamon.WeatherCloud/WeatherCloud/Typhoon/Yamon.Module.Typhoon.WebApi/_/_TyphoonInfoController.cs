...
//     如果想修改此文件的方法，请在此文件的上级目录中的TyphoonInfoController重写(override)该方法。
...
namespace Yamon.Module.Typhoon.WebApi
{
    ...
    /// 台风信息
    /// </summary>
    public partial class _TyphoonInfoController :BaseController
    {

        public TyphoonInfoDAL dal = new TyphoonInfoDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(TyphoonInfo model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.TyphoonID;

                    hash["id"] = newId;
                    if(returnData){
                        TyphoonInfo newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(TyphoonInfo model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.TyphoonID;
                    if(returnData){
                        TyphoonInfo newrow = dal.GetModelByID(model.TyphoonID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<TyphoonInfo> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<TyphoonInfo>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Typhoon_TyphoonInfo_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            TyphoonInfo model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
