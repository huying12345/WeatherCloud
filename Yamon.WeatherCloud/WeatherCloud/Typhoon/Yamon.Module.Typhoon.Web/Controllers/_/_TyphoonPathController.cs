...
//     如果想修改此文件的方法，请在此文件的上级目录中的TyphoonPathController重写(override)该方法。
...
namespace Yamon.Module.Typhoon.Web.Controllers
{
    ...
    /// 台风路径
    /// </summary>
    public partial class _TyphoonPathController :BaseController
    {

        public TyphoonPathDAL dal = new TyphoonPathDAL();
        ...
        public ActionResult Index(string m,string filterId)
        {
            ...
            ViewBag.DAL = new TyphoonPathDAL();
            ...
            string formPath = string.Format("~/Areas/Typhoon/Views/TyphoonPath/_/{0}.cshtml", m);
...
public virtual ActionResult Create(string id = "")
{
    ViewBag.ID = id;
    ViewBag.DAL=dal;
    TyphoonPath model= dal.GetCreateFormDefaultValue();
    model=dal.GetModelShowValue(model);
    string formPath = SiteCommon.GetCustomPage("~/Areas/Typhoon/Views/TyphoonPath/_/Create.cshtml");
...
public virtual ActionResult Edit(string id = "")
{
    ViewBag.ID = id;
    ViewBag.DAL=dal;
    TyphoonPath model = dal.GetModelByID(id);
    model=dal.GetEditFormDefaultValue(model);
    model=dal.GetModelShowValue(model);
    string formPath = SiteCommon.GetCustomPage("~/Areas/Typhoon/Views/TyphoonPath/_/Edit.cshtml");
    return View(formPath,model);
}public virtual ActionResult Show(string id = "")
{
    ...
    TyphoonPath model = dal.GetModelByID(id);
    model=dal.GetModelShowValue(model);
    string formPath = SiteCommon.GetCustomPage("~/Areas/Typhoon/Views/TyphoonPath/_/Show.cshtml");
    return View(formPath,model);
}
