...
//     如果想修改此文件的方法，请在此文件的上级目录中的zdzrainhourHistoryController重写(override)该方法。
...
namespace Yamon.Module.zdz.WebApi
{
    ...
    /// 一小时降雨量历史数据
    /// </summary>
    public partial class _zdzrainhourHistoryController :BaseController
    {

        public zdzrainhourHistoryDAL dal = new zdzrainhourHistoryDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(zdzrainhourHistory model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        zdzrainhourHistory newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(zdzrainhourHistory model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.id;
                    if(returnData){
                        zdzrainhourHistory newrow = dal.GetModelByID(model.id);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<zdzrainhourHistory> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<zdzrainhourHistory> modelList = new List<zdzrainhourHistory>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<zdzrainhourHistory>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"zdz_zdzrainhourHistory_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            zdzrainhourHistory model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
