...
//     如果想修改此文件的方法，请在此文件的上级目录中的zdzStationController重写(override)该方法。
...
namespace Yamon.Module.zdz.WebApi
{
    ...
    /// 自动站站点
    /// </summary>
    public partial class _zdzStationController :BaseController
    {

        public zdzStationDAL dal = new zdzStationDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(zdzStation model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.StationID;

                    hash["id"] = newId;
                    if(returnData){
                        zdzStation newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(zdzStation model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.StationID;
                    if(returnData){
                        zdzStation newrow = dal.GetModelByID(model.StationID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<zdzStation> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<zdzStation> modelList = new List<zdzStation>();
            switch(filterId)
            {
		...
		public  virtual ActionResult TreeGridByFilterID(string filterId)
        {
            ...
            try
            {
               ...
               List<zdzStation> newModelList = modelList.Select(model => dal.GetModelGridShowValue(model)).ToList();
        ...
        public  virtual ActionResult TreeJsonByFilterID(string filterId)
        {
            ...
            try
            {
                ...
                string fields="StationID,StationName,ParentID,OrderID";
                ...
                TreeHelper treeHelper = new TreeHelper("StationID", "StationName", "OrderID", "ParentID");
        ...
        public  virtual ActionResult AsyncTreeJsonByFilterID(string filterId)
        {
            ...
            try
            {
                ...
                switch(filterId)
                {
                     ...
                     default:
                     {
                        table = dal.GetAsyncTreeList(parentId,"StationID,StationName");
                        break;
                     }
                }
                TreeHelper treeHelper = new TreeHelper("StationID", "StationName", "OrderID", "ParentID");
        ...
        {
            int result = dal.UpdateSingleFieldValue("StationName", value,id );
            hash["success"] = result > 0;
            return Json(hash);
        }   
    [LogFilter]
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"zdz_zdzStation_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            zdzStation model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
