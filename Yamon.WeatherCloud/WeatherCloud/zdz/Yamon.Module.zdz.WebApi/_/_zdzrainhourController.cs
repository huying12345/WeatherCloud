...
//     如果想修改此文件的方法，请在此文件的上级目录中的zdzrainhourController重写(override)该方法。
...
namespace Yamon.Module.zdz.WebApi
{
    ...
    /// 一小时雨量
    /// </summary>
    public partial class _zdzrainhourController :BaseController
    {

        public zdzrainhourDAL dal = new zdzrainhourDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(zdzrainhour model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        zdzrainhour newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(zdzrainhour model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.id;
                    if(returnData){
                        zdzrainhour newrow = dal.GetModelByID(model.id);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<zdzrainhour> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<zdzrainhour> modelList = new List<zdzrainhour>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<zdzrainhour>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"zdz_zdzrainhour_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            zdzrainhour model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
