...
//     如果想修改此文件的方法，请在此文件的上级目录中的zdzinstantController重写(override)该方法。
...
namespace Yamon.Module.zdz.WebApi
{
    ...
    /// 自动站数据
    /// </summary>
    public partial class _zdzinstantController :BaseController
    {

        public zdzinstantDAL dal = new zdzinstantDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(zdzinstant model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        zdzinstant newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(zdzinstant model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        zdzinstant newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<zdzinstant> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<zdzinstant> modelList = new List<zdzinstant>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"zdz_zdzinstant_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            zdzinstant model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
